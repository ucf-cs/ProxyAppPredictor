...
files.
Additionally, this script can merge multiple datasets into one, which is useful 
if you have multiple CSVs for the same application.
"""
...
for file in glob.glob(PATH, recursive=True):
    ...
    if len(output[app_name]) == 0:
        output[app_name].extend(csv_data)
    else:
        assert len(output[app_name][0]) == len(csv_data[0])
        for i in range(len(output[app_name][0])):
            assert output[app_name][0][i] == csv_data[0][i]
        output[app_name].extend(csv_data[1:])

# Save the combined, corrected files.
for app in output:
    with open(app + "dataset.csv", "w", encoding="utf-8",
              newline="\n") as output_file:
        writer = csv.writer(output_file, delimiter=",", quotechar='"')
        for row in output[app]:
            writer.writerow(row)
